...
public final class Constraint {
    ...
    internal init(
        ...
        for layoutFromAttribute in layoutFromAttributes {
            ...
            let layoutToAttribute: LayoutAttribute
                ...
                if !layoutToAttributes.isEmpty {
                    if self.from.attributes == .edges, self.to.attributes == .margins {
                        ...
                        self.from.attributes == .directionalEdges,
                        self.to.attributes == .directionalMargins
                    {
                        switch layoutFromAttribute {
                        case .leading:
                        ...
                        self.from.attributes == .directionalMargins,
                        self.to.attributes == .directionalEdges
                    {
                        switch layoutFromAttribute {
                        case .leadingMargin:
                    ...
                    if
                        self.to.target == nil,
                        layoutFromAttribute == .centerX || layoutFromAttribute == .centerY
                    {
            ...
            // get layout constant
            let layoutConstant: CGFloat = self.constant
                .constraintConstantTargetValueFor(layoutAttribute: layoutToAttribute)
            ...
            // set priority
            layoutConstraint
                .priority = LayoutPriority(rawValue: self.priority.constraintPriorityTargetValue)
    ...
    internal func updateConstantAndPriorityIfNeeded() {
        for layoutConstraint in self.layoutConstraints {
            let attribute = (layoutConstraint.secondAttribute == .notAnAttribute) ? layoutConstraint
                .firstAttribute : layoutConstraint.secondAttribute
            layoutConstraint.constant = self.constant
                .constraintConstantTargetValueFor(layoutAttribute: attribute)

            let requiredPriority = ConstraintPriority.required.value
            if
                layoutConstraint.priority.rawValue < requiredPriority,
                self.priority.constraintPriorityTargetValue != requiredPriority
            {
                layoutConstraint
                    .priority = LayoutPriority(
                        rawValue: self.priority
                            .constraintPriorityTargetValue
                    )
            }
        }
    }
    ...
    internal func activateIfNeeded(updatingExisting: Bool = false) {
        guard let item = self.from.layoutConstraintItem else {
            print(
                "WARNING: SnapKit failed to get from item from constraint. Activate will be a no-op."
            )
            return
        }
        ...
        if updatingExisting {
            ...
            for layoutConstraint in layoutConstraints {
                let existingLayoutConstraint = existingLayoutConstraints
                    .first { $0 == layoutConstraint }
                guard let updateLayoutConstraint = existingLayoutConstraint else {
                    fatalError(
                        "Updated constraint could not find existing matching constraint to update: \(layoutConstraint)"
                    )
                }
                ...
                let updateLayoutAttribute = (
                    ...
                    .firstAttribute : updateLayoutConstraint.secondAttribute
                updateLayoutConstraint.constant = self.constant
                    .constraintConstantTargetValueFor(layoutAttribute: updateLayoutAttribute)
            }
        }
        else {
    ...
    internal func deactivateIfNeeded() {
        guard let item = self.from.layoutConstraintItem else {
            print(
                "WARNING: SnapKit failed to get from item from constraint. Deactivate will be a no-op."
            )
            return
        }
        let layoutConstraints = self.layoutConstraints
