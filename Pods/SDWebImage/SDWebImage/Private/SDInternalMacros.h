...
#define SD_LOCK_DECLARE(lock)                                                  \
  os_unfair_lock lock API_AVAILABLE(ios(10.0), tvos(10), watchos(3),           \
                                    macos(10.12));                             \
...
#ifndef SD_SEL_SPI
#define SD_SEL_SPI(name)                                                       \
  NSSelectorFromString([NSString stringWithFormat:@"_%@", SD_NSSTRING(name)])
#endif

#ifndef weakify
#define weakify(...)                                                           \
  sd_keywordify metamacro_foreach_cxt(sd_weakify_, , __weak, __VA_ARGS__)
#endif

#ifndef strongify
#define strongify(...)                                                         \
  sd_keywordify _Pragma("clang diagnostic push")                               \
...
#ifndef onExit
#define onExit                                                                 \
  sd_keywordify __strong sd_cleanupBlock_t metamacro_concat(sd_exitBlock_,     \
                                                            __LINE__)          \
