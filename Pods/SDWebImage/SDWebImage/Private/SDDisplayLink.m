...
- (instancetype)initWithTarget:(id)target selector:(SEL)sel {
  ...
  if (self) {
    ...
    _displayLink =
        [CADisplayLink displayLinkWithTarget:weakProxy
                                    selector:@selector(displayLinkDidRefresh:)];
#else
    SDWeakProxy *weakProxy = [SDWeakProxy proxyWithTarget:self];
    _displayLink =
        [NSTimer timerWithTimeInterval:kSDDisplayLinkInterval
                                target:weakProxy
                              selector:@selector(displayLinkDidRefresh:)
                              userInfo:nil
                               repeats:YES];
...
+ (instancetype)displayLinkWithTarget:(id)target selector:(SEL)sel {
  SDDisplayLink *displayLink = [[SDDisplayLink alloc] initWithTarget:target
                                                            selector:sel];
  return displayLink;
}
...
- (CFTimeInterval)duration {
  ...
  double periodPerSecond =
      (double)outputTime.videoTimeScale * outputTime.rateScalar;
...
#pragma clang diagnostic pop
  ...
  if (self.displayLink.isValid && self.currentFireDate != 0) {
    NSTimeInterval nextFireDate = CFRunLoopTimerGetNextFireDate(
        (__bridge CFRunLoopTimerRef)self.displayLink);
...
- (void)addToRunLoop:(NSRunLoop *)runloop forMode:(NSRunLoopMode)mode {
  ...
  CFRunLoopAddTimer(runloop.getCFRunLoop,
                    (__bridge CFRunLoopTimerRef)self.displayLink, cfMode);
#endif
}
...
- (void)removeFromRunLoop:(NSRunLoop *)runloop forMode:(NSRunLoopMode)mode {
  ...
  CFRunLoopRemoveTimer(runloop.getCFRunLoop,
                       (__bridge CFRunLoopTimerRef)self.displayLink, cfMode);
#endif
}
...
#if SD_MAC
  // CVDisplayLink does not use runloop, but we can provide similar behavior for
  // modes May use `default` runloop to avoid extra callback when in
  // `eventTracking` (mouse drag, scroll) or `modalPanel` (modal panel)
...
#pragma clang diagnostic pop
#if SD_WATCH
  self.currentFireDate = CFRunLoopTimerGetNextFireDate(
      (__bridge CFRunLoopTimerRef)self.displayLink);
#endif
}
