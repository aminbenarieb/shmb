...
// All of these method are called from the same global queue to avoid blocking
// on main queue and thread-safe problem. But it's also recommend to ensure
// thread-safe yourself using lock or other ways.
@required
/**
 Create a new disk cache based on the specified path. You can check
 `maxDiskSize` and `maxDiskAge` used for disk cache.
...
/**
 ...
 @return The value associated with key, or nil if no value is associated with
 key.
 */
...
/**
 ...
 @param key    The key with which to associate the value. If nil, this method
 has no effect.
 */
...
/**
 ...
 @discussion You can set any extended data to exist cache key. Without override
 the exist disk file data. on UNIX, the common way for this is to use the
...
/**
 ...
 @param key The key identifying the value to be removed. If nil, this method has
 no effect.
 */
...
/**
 Removes the expired data from the cache. You can choose the data to remove base
 on `ageLimit`, `countLimit` and `sizeLimit` options.
 */
...
/**
 Move the cache directory from old location to new location, the old location
 will be removed after finish. If the old location does not exist, does nothing.
 If the new location does not exist, only do a movement of directory.
 If the new location does exist, will move and merge the files from old
 location. If the new location does exist, but is not a directory, will remove
 it and do a movement of directory.
