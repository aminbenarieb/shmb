...
@property(nonatomic, copy, nonnull)
    SDWebImageDownloaderResponseModifierBlock block;
...
+ (instancetype)responseModifierWithBlock:
    (SDWebImageDownloaderResponseModifierBlock)block {
  SDWebImageDownloaderResponseModifier *responseModifier =
      [[SDWebImageDownloaderResponseModifier alloc] initWithBlock:block];
...
- (instancetype)initWithStatusCode:(NSInteger)statusCode
  ...
  return [self initWithBlock:^NSURLResponse *_Nullable(
                   NSURLResponse *_Nonnull response) {
    if (![response isKindOfClass:NSHTTPURLResponse.class]) {
      return response;
    }
    NSMutableDictionary *mutableHeaders =
        [((NSHTTPURLResponse *)response).allHeaderFields mutableCopy];
    ...
    NSHTTPURLResponse *httpResponse =
        [[NSHTTPURLResponse alloc] initWithURL:response.URL
                                   ...
                                   HTTPVersion:version
                                  headerFields:[mutableHeaders copy]];
    return httpResponse;
  }];
}
