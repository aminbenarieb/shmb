...
- (NSUInteger)sd_imageLoopCount {
  ...
  NSNumber *value =
      objc_getAssociatedObject(self, @selector(sd_imageLoopCount));
...
- (void)setSd_imageLoopCount:(NSUInteger)sd_imageLoopCount {
  ...
  objc_setAssociatedObject(self, @selector(sd_imageLoopCount), value,
...
- (BOOL)sd_isVector {
  if (@available(iOS 13.0, tvOS 13.0, watchOS 6.0, *)) {
    ...
    if ([self respondsToSelector:SymbolSelector] &&
        [self performSelector:SymbolSelector]) {
      return YES;
    }
    // SVG
    SEL SVGSelector = SD_SEL_SPI(CGSVGDocument);
    if ([self respondsToSelector:SVGSelector] &&
        [self performSelector:SVGSelector]) {
      return YES;
    }
  }
  if (@available(iOS 11.0, tvOS 11.0, watchOS 4.0, *)) {
    ...
    if ([self respondsToSelector:PDFSelector] &&
        [self performSelector:PDFSelector]) {
      return YES;
    }
  }
  return NO;
...
- (NSUInteger)sd_imageLoopCount {
  ...
  NSImageRep *imageRep = [self bestRepresentationForRect:imageRect
                                                 context:nil
                                                   hints:nil];
  ...
  if (bitmapImageRep) {
    imageLoopCount = [[bitmapImageRep valueForProperty:NSImageLoopCount]
...
- (void)setSd_imageLoopCount:(NSUInteger)sd_imageLoopCount {
  ...
  if (bitmapImageRep) {
    [bitmapImageRep setProperty:NSImageLoopCount
                      withValue:@(sd_imageLoopCount)];
  }
}
...
- (BOOL)sd_isAnimated {
  ...
  if (bitmapImageRep) {
    NSUInteger frameCount = [[bitmapImageRep valueForProperty:NSImageFrameCount]
        unsignedIntegerValue];
...
- (void)setSd_imageFormat:(SDImageFormat)sd_imageFormat {
  objc_setAssociatedObject(self, @selector(sd_imageFormat), @(sd_imageFormat),
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (void)setSd_isIncremental:(BOOL)sd_isIncremental {
  objc_setAssociatedObject(self, @selector(sd_isIncremental),
                           @(sd_isIncremental),
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
