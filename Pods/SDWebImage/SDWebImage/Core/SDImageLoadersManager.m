...
@property(nonatomic, strong, nonnull)
    NSMutableArray<id<SDImageLoader>> *imageLoaders;
...
- (instancetype)init {
  ...
  if (self) {
    ...
    _imageLoaders = [NSMutableArray
        arrayWithObject:[SDWebImageDownloader sharedDownloader]];
...
- (id<SDWebImageOperation>)
  ...
  for (id<SDImageLoader> loader in loaders.reverseObjectEnumerator) {
    if ([loader canRequestImageForURL:url]) {
      return [loader requestImageWithURL:url
                                 options:options
                                 context:context
