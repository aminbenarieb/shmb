...
/**
 * Called when an image was prefetched. Which means it's called when one URL
 * from any of prefetching finished.
 *
...
/**
 * Called when all images are prefetched. Which means it's called when all URLs
 * from all of prefetching finished.
 * @param imagePrefetcher The current image prefetcher
 * @param totalCount      The total number of images that were prefetched
 ...
 * @note You can specify a standalone manager and downloader with custom
 * configuration suitable for image prefetching. Such as `currentDownloadCount`
 * or `downloadTimeout`.
 */
...
/**
 * Queue options for prefetcher when call the progressBlock, completionBlock and
 * delegate methods. Defaults to Main Queue.
 * @note The call is asynchronously to avoid blocking target queue.
 * @note The delegate queue should be set before any prefetching start and may
 * not be changed during prefetching to avoid thread-safe problem.
 */
...
/**
 * Returns the global shared image prefetcher instance. It use a standalone
 * manager which is different from shared manager.
 */
...
/**
 * Assign list of URLs to let SDWebImagePrefetcher to queue the prefetching. It
 * based on the image manager so the image may from the cache and network
 * according to the `options` property. Prefetching is separate to each other,
 * which means the progressBlock and completionBlock you provide is bind to the
 * prefetching for the list of urls. Attention that call this will not cancel
 * previous fetched urls. You should keep the token return by this to cancel or
 * cancel all the prefetch.
 *
 ...
 *                        first parameter is the number of completed (successful
 * or not) requests, second parameter is the total number of images originally
 * requested to be prefetched
 * @param completionBlock block to be called when the current prefetching is
 * completed first param is the number of completed (successful or not)
 * requests, second parameter is the number of skipped requests
 * @return the token to cancel the current prefetching.
 */
