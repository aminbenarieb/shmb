...
NSNotificationName const SDWebImageDownloadStartNotification =
    @"SDWebImageDownloadStartNotification";
NSNotificationName const SDWebImageDownloadReceiveResponseNotification =
    @"SDWebImageDownloadReceiveResponseNotification";
NSNotificationName const SDWebImageDownloadStopNotification =
    @"SDWebImageDownloadStopNotification";
NSNotificationName const SDWebImageDownloadFinishNotification =
...
@property(nonatomic, weak, nullable)
    NSOperation<SDWebImageDownloaderOperation> *downloadOperation;
@property(nonatomic, assign, getter=isCancelled) BOOL cancelled;

- (nonnull instancetype)init NS_UNAVAILABLE;
+ (nonnull instancetype)new NS_UNAVAILABLE;
- (nonnull instancetype)initWithDownloadOperation:
    (nullable NSOperation<SDWebImageDownloaderOperation> *)downloadOperation;

@end

@interface SDWebImageDownloader () <NSURLSessionTaskDelegate,
                                    NSURLSessionDataDelegate>

@property(strong, nonatomic, nonnull) NSOperationQueue *downloadQueue;
@property(strong, nonatomic, nonnull)
    NSMutableDictionary<NSURL *, NSOperation<SDWebImageDownloaderOperation> *>
        *URLOperations;
...
@implementation SDWebImageDownloader {
  SD_LOCK_DECLARE(_HTTPHeadersLock); // A lock to keep the access to
                                     // `HTTPHeaders` thread-safe
  SD_LOCK_DECLARE(_operationsLock);  // A lock to keep the access to
                                     // `URLOperations` thread-safe
}

+ (void)initialize {
  // Bind SDNetworkActivityIndicator if available (download it here:
  // http://github.com/rs/SDNetworkActivityIndicator ) To use it, just add
  // #import "SDNetworkActivityIndicator.h" in addition to the SDWebImage import
...
#pragma clang diagnostic pop
    ...
    [[NSNotificationCenter defaultCenter]
        removeObserver:activityIndicator
...
- (nonnull instancetype)init {
  return
      [self initWithConfig:SDWebImageDownloaderConfig.defaultDownloaderConfig];
}
...
- (instancetype)initWithConfig:(SDWebImageDownloaderConfig *)config {
  ...
  if (self) {
    ...
    [_config addObserver:self
              forKeyPath:NSStringFromSelector(@selector(maxConcurrentDownloads))
                 options:0
                 context:SDWebImageDownloaderContext];
    _downloadQueue = [NSOperationQueue new];
    _downloadQueue.maxConcurrentOperationCount = _config.maxConcurrentDownloads;
    _downloadQueue.name = @"com.hackemist.SDWebImageDownloader";
    _URLOperations = [NSMutableDictionary new];
    NSMutableDictionary<NSString *, NSString *> *headerDictionary =
     ...
     *  We send nil as delegate queue so that the session creates a serial
...
- (nullable SDWebImageDownloadToken *)
  ...
  // The URL will be used as the key to the callbacks dictionary so it cannot be
  // nil. If it is nil immediately call the completed block with no image or
  // data.
  if (url == nil) {
    if (completedBlock) {
  ...
  NSOperation<SDWebImageDownloaderOperation> *operation =
      [self.URLOperations objectForKey:url];
  // There is a case that the operation may be marked as finished or cancelled,
  // but not been removed from `self.URLOperations`.
  if (!operation || operation.isFinished || operation.isCancelled) {
    ...
    // Add the handlers before submitting to operation queue, avoid the race
    // condition that operation finished before setting handlers.
    downloadOperationCancelToken =
        [operation addHandlersForProgress:progressBlock
                                completed:completedBlock];
    // Add operation to operation queue only after all configuration done
    // according to Apple's doc. `addOperation:` does not synchronously execute
    // the `operation.completionBlock` so this will not cause deadlock.
    [self.downloadQueue addOperation:operation];
  } else {
    // When we reuse the download operation to attach more callbacks, there may
    // be thread safe issue because the getter of callbacks may in another queue
    // (decoding queue or delegate queue) So we lock the operation here, and in
    // `SDWebImageDownloaderOperation`, we use `@synchonzied (self)`, to ensure
    // the thread safe between these two classes.
  ...
  SDWebImageDownloadToken *token =
      [[SDWebImageDownloadToken alloc] initWithDownloadOperation:operation];
  token.url = url;
...
- (nullable NSOperation<SDWebImageDownloaderOperation> *)
    createDownloaderOperationWithUrl:(nonnull NSURL *)url
                             options:(SDWebImageDownloaderOptions)options
                             context:(nullable SDWebImageContext *)context {
  ...
  // In order to prevent from potential duplicate caching (NSURLCache +
  // SDImageCache) we disable the cache for image requests if told otherwise
  ...
  mutableRequest.HTTPShouldHandleCookies =
      SD_OPTIONS_CONTAINS(options, SDWebImageDownloaderHandleCookies);
  ...
  if ([context valueForKey:SDWebImageContextDownloadRequestModifier]) {
    requestModifier =
        [context valueForKey:SDWebImageContextDownloadRequestModifier];
  } else {
  ...
  if (requestModifier) {
    NSURLRequest *modifiedRequest =
        [requestModifier modifiedRequestWithRequest:[mutableRequest copy]];
  ...
  if (responseModifier) {
    mutableContext[SDWebImageContextDownloadResponseModifier] =
        responseModifier;
  }
  // Decryptor
  ...
  if (operationClass &&
      ...
      [operationClass
          conformsToProtocol:@protocol(SDWebImageDownloaderOperation)]) {
    // Custom operation class
  } else {
    operationClass = [SDWebImageDownloaderOperation class];
  }
  NSOperation<SDWebImageDownloaderOperation> *operation =
      [[operationClass alloc] initWithRequest:request
  ...
  if ([operation respondsToSelector:@selector(setCredential:)]) {
    if (self.config.urlCredential) {
      ...
      operation.credential = [NSURLCredential
          credentialWithUser:self.config.username
  ...
  if ([operation respondsToSelector:@selector(setMinimumProgressInterval:)]) {
    operation.minimumProgressInterval =
        MIN(MAX(self.config.minimumProgressInterval, 0), 1);
  }
  ...
  if (self.config.executionOrder == SDWebImageDownloaderLIFOExecutionOrder) {
    // Emulate LIFO execution order by systematically, each previous adding
    // operation can dependency the new operation This can gurantee the new
    // operation to be execulated firstly, even if when some operations
    // finished, meanwhile you appending new operations Just make last added
    // operation dependents new operation can not solve this problem. See test
    // case #test15DownloaderLIFOExecutionOrder
...
- (void)observeValueForKeyPath:(NSString *)keyPath
  ...
  if (context == SDWebImageDownloaderContext) {
    if ([keyPath isEqualToString:NSStringFromSelector(
      ...
      self.downloadQueue.maxConcurrentOperationCount =
          self.config.maxConcurrentDownloads;
    }
  } else {
...
- (NSOperation<SDWebImageDownloaderOperation> *)operationWithTask:
    (NSURLSessionTask *)task {
  NSOperation<SDWebImageDownloaderOperation> *returnOperation = nil;
  for (NSOperation<SDWebImageDownloaderOperation> *operation in self
           .downloadQueue.operations) {
    if ([operation respondsToSelector:@selector(dataTask)]) {
      // So we lock the operation here, and in `SDWebImageDownloaderOperation`,
      // we use `@synchonzied (self)`, to ensure the thread safe between these
      // two classes.
...
- (void)URLSession:(NSURLSession *)session
  ...
  NSOperation<SDWebImageDownloaderOperation> *dataOperation =
      [self operationWithTask:dataTask];
  if ([dataOperation respondsToSelector:@selector
                     (URLSession:
...
- (void)dealloc {
  [[NSNotificationCenter defaultCenter]
      removeObserver:self
                name:SDWebImageDownloadReceiveResponseNotification
              object:nil];
  [[NSNotificationCenter defaultCenter]
      removeObserver:self
                name:SDWebImageDownloadStopNotification
...
- (instancetype)initWithDownloadOperation:
    (NSOperation<SDWebImageDownloaderOperation> *)downloadOperation {
  self = [super init];
  if (self) {
    ...
    [[NSNotificationCenter defaultCenter]
        addObserver:self
           selector:@selector(downloadDidReceiveResponse:)
               name:SDWebImageDownloadReceiveResponseNotification
             object:downloadOperation];
    [[NSNotificationCenter defaultCenter]
        addObserver:self
           selector:@selector(downloadDidStop:)
               name:SDWebImageDownloadStopNotification
             object:downloadOperation];
  }
  return self;
...
- (BOOL)shouldBlockFailedURLWithURL:(NSURL *)url error:(NSError *)error {
  return [self shouldBlockFailedURLWithURL:url
                                     error:error
                                   options:0
                                   context:nil];
}
