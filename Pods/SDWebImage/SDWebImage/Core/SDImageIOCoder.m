...
- (UIImage *)decodedImageWithData:(NSData *)data
  ...
  UIImage *image =
      [SDImageIOAnimatedCoder createFrameAtIndex:0
                                          source:source
                                           scale:scale
...
- (void)updateIncrementalData:(NSData *)data finished:(BOOL)finished {
  ...
  if (_width + _height == 0) {
    CFDictionaryRef properties =
        CGImageSourceCopyPropertiesAtIndex(_imageSource, 0, NULL);
    if (properties) {
...
- (UIImage *)incrementalDecodedImageWithOptions:(SDImageCoderOptions *)options {
  ...
  if (_width + _height > 0) {
    ...
    image = [SDImageIOAnimatedCoder createFrameAtIndex:0
                                                source:_imageSource
                                                 scale:scale
...
- (NSData *)encodedDataWithImage:(UIImage *)image
  ...
  CGImageDestinationRef imageDestination = CGImageDestinationCreateWithData(
      (__bridge CFMutableDataRef)imageData, imageUTType, 1, NULL);
  if (!imageDestination) {
    // Handle failure.
    return nil;
  }
  ...
  CGImagePropertyOrientation exifOrientation = [SDImageCoderHelper
      exifOrientationFromImageOrientation:image.imageOrientation];
#else
