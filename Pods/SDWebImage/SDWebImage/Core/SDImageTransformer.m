...
// Separator for different transformerKey, for example, `image.png` |>
// flip(YES,NO) |> rotate(pi/4,YES) =>
...
NSString *_Nullable SDTransformedKeyForKey(NSString *_Nullable key,
                                           NSString *_Nonnull transformerKey) {
  ...
  if (ext.length > 0) {
    ...
    if (keyURL && !keyURL.isFileURL) {
      ...
      NSURLComponents *component = [NSURLComponents componentsWithURL:keyURL
                                              resolvingAgainstBaseURL:NO];
      component.path = [[[component.path.stringByDeletingPathExtension
          stringByAppendingString:SDImageTransformerKeySeparator]
          stringByAppendingString:transformerKey]
    ...
    return [[key stringByAppendingString:SDImageTransformerKeySeparator]
        stringByAppendingString:transformerKey];
  }
}
...
NSString *_Nullable SDThumbnailedKeyForKey(NSString *_Nullable key,
  ...
  NSString *thumbnailKey = [NSString
      stringWithFormat:@"Thumbnail({%f,%f},%d)", thumbnailPixelSize.width,
                       thumbnailPixelSize.height, preserveAspectRatio];
...
@property(nonatomic, copy, readwrite, nonnull)
    NSArray<id<SDImageTransformer>> *transformers;
...
+ (instancetype)transformerWithTransformers:
    (NSArray<id<SDImageTransformer>> *)transformers {
  ...
  transformer.transformerKey =
      [[self class] cacheKeyForTransformers:transformers];

  return transformer;
}

+ (NSString *)cacheKeyForTransformers:
    (NSArray<id<SDImageTransformer>> *)transformers {
  if (transformers.count == 0) {
    return @"";
  }
  NSMutableArray<NSString *> *cacheKeys =
      [NSMutableArray arrayWithCapacity:transformers.count];
  [transformers
      enumerateObjectsUsingBlock:^(id<SDImageTransformer> _Nonnull transformer,
...
- (UIImage *)transformedImageWithImage:(UIImage *)image forKey:(NSString *)key {
  ...
  for (id<SDImageTransformer> transformer in self.transformers) {
    transformedImage = [transformer transformedImageWithImage:transformedImage
                                                       forKey:key];
  }
...
+ (instancetype)transformerWithRadius:(CGFloat)cornerRadius
                              corners:(SDRectCorner)corners
                          borderWidth:(CGFloat)borderWidth
                          borderColor:(UIColor *)borderColor {
  SDImageRoundCornerTransformer *transformer =
      [SDImageRoundCornerTransformer new];
...
- (NSString *)transformerKey {
  return [NSString
      stringWithFormat:@"SDImageRoundCornerTransformer(%f,%lu,%f,%@)",
                       self.cornerRadius, (unsigned long)self.corners,
                       self.borderWidth, self.borderColor.sd_hexString];
}
...
- (UIImage *)transformedImageWithImage:(UIImage *)image forKey:(NSString *)key {
  ...
  return [image sd_roundedCornerImageWithRadius:self.cornerRadius
...
+ (instancetype)transformerWithSize:(CGSize)size
...
- (NSString *)transformerKey {
  ...
  return [NSString stringWithFormat:@"SDImageResizingTransformer({%f,%f},%lu)",
                                    size.width, size.height,
                                    (unsigned long)self.scaleMode];
}
...
- (NSString *)transformerKey {
  ...
  return
      [NSString stringWithFormat:@"SDImageCroppingTransformer({%f,%f,%f,%f})",
...
+ (instancetype)transformerWithHorizontal:(BOOL)horizontal
...
- (NSString *)transformerKey {
  return [NSString stringWithFormat:@"SDImageFlippingTransformer(%d,%d)",
...
- (UIImage *)transformedImageWithImage:(UIImage *)image forKey:(NSString *)key {
  ...
  return [image sd_flippedImageWithHorizontal:self.horizontal
...
- (NSString *)transformerKey {
  return [NSString stringWithFormat:@"SDImageRotationTransformer(%f,%d)",
                                    self.angle, self.fitSize];
}
...
- (NSString *)transformerKey {
  return [NSString stringWithFormat:@"SDImageTintTransformer(%@)",
                                    self.tintColor.sd_hexString];
}
...
- (NSString *)transformerKey {
  return [NSString
      stringWithFormat:@"SDImageBlurTransformer(%f)", self.blurRadius];
}
...
- (NSString *)transformerKey {
  return [NSString
      stringWithFormat:@"SDImageFilterTransformer(%@)", self.filter.name];
}
