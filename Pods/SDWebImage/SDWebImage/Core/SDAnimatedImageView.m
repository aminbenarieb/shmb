...
#if SD_UIKIT
@dynamic animationRepeatCount; // we re-use this property from `UIImageView`
                               // super class on iOS.
#endif
...
+ (instancetype)imageViewWithImage:(NSImage *)image {
  ...
  SDAnimatedImageView *imageView =
      [[SDAnimatedImageView alloc] initWithFrame:frame];
  [imageView setImage:image];
  return imageView;
}
#else
// -initWithImage: isn't documented as a designated initializer of UIImageView,
// but it actually seems to be. Using -initWithImage: doesn't call any of the
...
// -initWithImage:highlightedImage: also isn't documented as a designated
// initializer of UIImageView, but it doesn't call any other designated
// initializers.
- (instancetype)initWithImage:(UIImage *)image
...
- (void)commonInit {
  // Pay attention that UIKit's `initWithImage:` will trigger a `setImage:`
  // during initialization before this `commonInit`. So the properties which
  // rely on this order, should using lazy-evaluation or do extra check in
  // `setImage:`.
...
- (void)setImage:(UIImage *)image {
  ...
  // We need call super method to keep function. This will impliedly call
  // `setNeedsDisplay`. But we have no way to avoid this when using animated
  // image. So we call `setNeedsDisplay` again at the end.
  super.image = image;
  if ([image.class conformsToProtocol:@protocol(SDAnimatedImage)]) {
    if (!self.player) {
      ...
      self.player.totalFrameCount =
          [(id<SDAnimatedImage>)image animatedImageFrameCount];
    }

    if (!self.player) {
      // animated player nil means the image format is not supported, or frame
      // count <= 1
      return;
    }
    ...
    self.player.animationLoopHandler = ^(NSUInteger loopCount) {
      @strongify(self);
      // Progressive image reach the current last frame index. Keep the state
      // and pause animating. Wait for later restart
      if (self.isProgressive) {
    ...
    // Ensure disabled highlighting; it's not supported (see
    // `-setHighlighted:`).
...
+ (NSString *)defaultRunLoopMode {
  // Key off `activeProcessorCount` (as opposed to `processorCount`) since the
  // system could shut down cores in certain situations.
...
- (void)setHighlighted:(BOOL)highlighted {
  // Highlighted image is unsupported for animated images, but implementing it
  // breaks the image view when embedded in a UICollectionViewCell.
  if (!self.player) {
...
- (void)checkPlay {
  // Only handle for SDAnimatedImage, leave UIAnimatedImage or animationImages
  // for super implementation control
...
// Don't repeatedly check our window & superview in `-displayDidRefresh:` for
// performance reasons. Just update our cached value whenever the animated image
// or visibility (window, superview, hidden, alpha) is changed.
- (void)updateShouldAnimate {
#if SD_MAC
  BOOL isVisible = self.window && self.superview && ![self isHidden] &&
                   self.alphaValue > 0.0;
#else
  BOOL isVisible =
      self.window && self.superview && ![self isHidden] && self.alpha > 0.0;
#endif
...
- (void)updateIsProgressiveWithImage:(UIImage *)image {
  ...
  // We must use `image.class conformsToProtocol:` instead of `image
  // conformsToProtocol:` here Because UIKit on macOS, using internal hard-coded
  // override method, which returns NO
  id<SDAnimatedImageCoder> currentAnimatedCoder =
      [self progressiveAnimatedCoderForImage:image];
  if (currentAnimatedCoder) {
    UIImage *previousImage = self.image;
    if (!previousImage) {
      // If current animated coder supports progressive, and no previous image
      // to check, start progressive loading
      self.isProgressive = YES;
    } else {
      id<SDAnimatedImageCoder> previousAnimatedCoder =
          [self progressiveAnimatedCoderForImage:previousImage];
      if (previousAnimatedCoder == currentAnimatedCoder) {
        // If current animated coder is the same as previous, start progressive
        // loading
        self.isProgressive = YES;
      }
    }
...
- (id<SDAnimatedImageCoder, SDProgressiveImageCoder>)
    progressiveAnimatedCoderForImage:(UIImage *)image {
  if ([image.class conformsToProtocol:@protocol(SDAnimatedImage)] &&
      image.sd_isIncremental &&
      [image respondsToSelector:@selector(animatedCoder)]) {
    id<SDAnimatedImageCoder> animatedCoder =
        [(id<SDAnimatedImage>)image animatedCoder];
...
- (void)displayLayer:(CALayer *)layer {
  ...
  if (currentFrame) {
    ...
    // If we have no animation frames, call super implementation. iOS 14+
    // UIImageView use this delegate method for rendering.
...
// Why using this design may because of properties like `imageAlignment` and
// `imageScaling`, which it's not available for UIImageView.contentMode (it's
// impossible to align left and keep aspect ratio at the same time)
- (NSView *)imageView {
  NSImageView *imageView =
      objc_getAssociatedObject(self, SD_SEL_SPI(imageView));
  if (!imageView) {
    // macOS 10.14
...
// on macOS, it's the imageView subview's layer (we use layer-hosting view to
// let CALayerDelegate works)
