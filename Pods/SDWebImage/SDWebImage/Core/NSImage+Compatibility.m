...
- (instancetype)initWithCGImage:(nonnull CGImageRef)cgImage
  ...
  if (orientation != kCGImagePropertyOrientationUp) {
    // AppKit design is different from UIKit. Where CGImage based image rep does
    // not respect to any orientation. Only data based image rep which contains
    // the EXIF metadata can automatically detect orientation. This should be
    // nonnull, until the memory is exhausted cause `CGBitmapContextCreate`
    // failed.
    CGImageRef rotatedCGImage =
        [SDImageCoderHelper CGImageCreateDecoded:cgImage
...
- (instancetype)initWithCIImage:(nonnull CIImage *)ciImage
                          scale:(CGFloat)scale
                    orientation:(CGImagePropertyOrientation)orientation {
