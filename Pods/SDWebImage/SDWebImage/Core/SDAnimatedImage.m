...
static CGFloat SDImageScaleFromPath(NSString *string) {
  ...
  NSRegularExpression *pattern = [NSRegularExpression
      regularExpressionWithPattern:@"@[0-9]+\\.?[0-9]*x$"
                           options:NSRegularExpressionAnchorsMatchLines
                             error:nil];
  [pattern
      enumerateMatchesInString:name
                       options:kNilOptions
...
@property(atomic, copy) NSArray<SDImageFrame *>
    *loadedAnimatedImageFrames; // Mark as atomic to keep thread-safe
...
+ (instancetype)imageNamed:(NSString *)name
  ...
  NSString *path = [assetManager getPathForName:name
                                         bundle:bundle
...
- (instancetype)initWithData:(NSData *)data
                       scale:(CGFloat)scale
                     options:(SDImageCoderOptions *)options {
  ...
  for (id<SDImageCoder> coder in [SDImageCodersManager sharedManager]
    ...
    if ([coder conformsToProtocol:@protocol(SDAnimatedImageCoder)]) {
      if ([coder canDecodeFromData:data]) {
        ...
        animatedCoder =
            [[[coder class] alloc] initWithAnimatedImageData:data
                                                     options:options];
        break;
      }
    }
...
- (instancetype)initWithAnimatedCoder:(id<SDAnimatedImageCoder>)animatedCoder
                                scale:(CGFloat)scale {
  if (!animatedCoder) {
    return nil;
  }
...
#endif
  if (self) {
    // Only keep the animated coder if frame count > 1, save RAM usage for
    // non-animated image format (APNG/WebP)
...
- (void)preloadAllFrames {
  ...
  if (!self.isAllFramesLoaded) {
    NSMutableArray<SDImageFrame *> *frames =
        [NSMutableArray arrayWithCapacity:self.animatedImageFrameCount];
    for (size_t i = 0; i < self.animatedImageFrameCount; i++) {
      ...
      SDImageFrame *frame = [SDImageFrame
          frameWithImage:image
                duration:duration]; // through the image should be nonnull, used
                                    // as nullable for
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
  ...
  if (self) {
    _animatedImageFormat =
        [aDecoder decodeIntegerForKey:NSStringFromSelector(
                                          @selector(animatedImageFormat))];
    NSData *animatedImageData = [aDecoder
        decodeObjectOfClass:[NSData class]
                     forKey:NSStringFromSelector(@selector(animatedImageData))];
    ...
    for (id<SDImageCoder> coder in [SDImageCodersManager sharedManager]
      ...
      if ([coder conformsToProtocol:@protocol(SDAnimatedImageCoder)]) {
        if ([coder canDecodeFromData:animatedImageData]) {
          animatedCoder = [[[coder class] alloc]
              initWithAnimatedImageData:animatedImageData
                                options:@{
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
  ...
  [aCoder encodeInteger:self.animatedImageFormat
                 forKey:NSStringFromSelector(@selector(animatedImageFormat))];
  NSData *animatedImageData = self.animatedImageData;
  if (animatedImageData) {
    [aCoder encodeObject:animatedImageData
                  forKey:NSStringFromSelector(@selector(animatedImageData))];
  }
}
...
- (NSUInteger)sd_memoryCost {
  ...
  NSUInteger bytesPerFrame =
      CGImageGetBytesPerRow(imageRef) * CGImageGetHeight(imageRef);
...
+ (nullable UIImage *)sd_imageWithData:(nullable NSData *)data
  ...
  return [[self alloc]
      initWithData:data
             scale:scale
           options:@{SDImageCoderDecodeFirstFrameOnly : @(firstFrameOnly)}];
}
...
- (nullable NSData *)sd_imageDataAsFormat:(SDImageFormat)imageFormat
  ...
  return [self sd_imageDataAsFormat:imageFormat
...
- (nullable NSData *)sd_imageDataAsFormat:(SDImageFormat)imageFormat
  ...
  if (firstFrameOnly) {
    ...
    return [super sd_imageDataAsFormat:imageFormat
                    compressionQuality:compressionQuality
  ...
  NSData *imageData = [animatedImage sd_imageDataAsFormat:imageFormat
                                       compressionQuality:compressionQuality
                                           firstFrameOnly:firstFrameOnly];
  return imageData;
}
