...
+ (instancetype)decryptorWithBlock:(SDWebImageDownloaderDecryptorBlock)block {
  SDWebImageDownloaderDecryptor *decryptor =
      [[SDWebImageDownloaderDecryptor alloc] initWithBlock:block];
  return decryptor;
}
...
+ (SDWebImageDownloaderDecryptor *)base64Decryptor {
  ...
  dispatch_once(&onceToken, ^{
    decryptor = [SDWebImageDownloaderDecryptor
        decryptorWithBlock:^NSData *_Nullable(
            NSData *_Nonnull data, NSURLResponse *_Nullable response) {
          NSData *modifiedData = [[NSData alloc]
              initWithBase64EncodedData:data
                                options:
