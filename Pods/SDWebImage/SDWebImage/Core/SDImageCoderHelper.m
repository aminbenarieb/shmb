...
/*
 * Defines the maximum size in MB of the decoded image when the flag
 * `SDWebImageScaleDownLargeImages` is set Suggested value for iPad1 and iPhone
 * 3GS: 60. Suggested value for iPad2 and iPhone 4: 120. Suggested value for
 * iPhone 3G and iPod 2 and earlier devices: 30.
 */
#if SD_MAC
...
static const CGFloat kDestSeemOverlap =
...
+ (UIImage *)animatedImageWithFrames:(NSArray<SDImageFrame *> *)frames {
  ...
  [frames enumerateObjectsUsingBlock:^(SDImageFrame *_Nonnull frame,
  ...
  animatedImage = [UIImage animatedImageWithImages:animatedImages
                                          duration:totalDuration / 1000.f];
  ...
  CGImageDestinationRef imageDestination = CGImageDestinationCreateWithData(
      (__bridge CFMutableDataRef)imageData, imageUTType, frameCount, NULL);
  if (!imageDestination) {
    // Handle failure.
    return nil;
  }
  ...
  SDAnimatedImageRep *imageRep =
      [[SDAnimatedImageRep alloc] initWithData:imageData];
  NSSize size =
...
+ (NSArray<SDImageFrame *> *)framesFromAnimatedImage:(UIImage *)animatedImage {
  ...
  if (avgDuration == 0) {
    avgDuration = 0.1; // if it's a animated image but no duration, set it to
                       // default 100ms (this do not have that 10ms limit like
                       // GIF or WebP to allow custom coder provide the limit)
  }
  ...
  [animatedImages
      enumerateObjectsUsingBlock:^(UIImage *_Nonnull image, NSUInteger idx,
                                   BOOL *_Nonnull stop) {
        // ignore first
        if (idx == 0) {
          return;
        ...
        if ([image isEqual:previousImage]) {
          ...
          SDImageFrame *frame =
              [SDImageFrame frameWithImage:previousImage
                                  duration:avgDuration * repeatCount];
  ...
  NSRect imageRect =
      NSMakeRect(0, 0, animatedImage.size.width, animatedImage.size.height);
  NSImageRep *imageRep = [animatedImage bestRepresentationForRect:imageRect
                                                          context:nil
                                                            hints:nil];
  ...
  for (size_t i = 0; i < frameCount; i++) {
    @autoreleasepool {
      ...
      NSTimeInterval frameDuration = [[bitmapImageRep
          valueForProperty:NSImageCurrentFrameDuration] doubleValue];
      NSImage *frameImage =
          [[NSImage alloc] initWithCGImage:bitmapImageRep.CGImage
                                     scale:scale
                               orientation:kCGImagePropertyOrientationUp];
      SDImageFrame *frame = [SDImageFrame frameWithImage:frameImage
...
+ (CGColorSpaceRef)colorSpaceGetDeviceRGB {
  ...
  CGColorSpaceRef screenColorSpace =
      NSScreen.mainScreen.colorSpace.CGColorSpace;
  if (screenColorSpace) {
...
+ (CGImageRef)CGImageCreateDecoded:(CGImageRef)cgImage {
  return [self CGImageCreateDecoded:cgImage
                        orientation:kCGImagePropertyOrientationUp];
}
...
+ (CGImageRef)CGImageCreateDecoded:(CGImageRef)cgImage
  ...
  // iOS prefer BGRA8888 (premultiplied) or BGRX8888 bitmapInfo for screen
  // rendering, which is same as `UIGraphicsBeginImageContext()` or `- [CALayer
  // drawInContext:]` Though you can use any supported bitmapInfo (see:
  ...
  // ) and let Core Graphics reorder it when you call `CGContextDrawImage` But
  // since our build-in coders use this bitmapInfo, this can have a little
  // performance benefit
  CGBitmapInfo bitmapInfo = kCGBitmapByteOrder32Host;
  bitmapInfo |=
      hasAlpha ? kCGImageAlphaPremultipliedFirst : kCGImageAlphaNoneSkipFirst;
  CGContextRef context =
      CGBitmapContextCreate(NULL, newWidth, newHeight, 8, 0,
  ...
  // Apply transform
  CGAffineTransform transform = SDCGContextTransformFromOrientation(
      orientation, CGSizeMake(newWidth, newHeight));
  CGContextConcatCTM(context, transform);
  CGContextDrawImage(context, CGRectMake(0, 0, width, height),
                     cgImage); // The rect is bounding box of CGImage, don't
                               // swap width & height
...
+ (CGImageRef)CGImageCreateScaled:(CGImageRef)cgImage size:(CGSize)size {
  ...
  vImage_Error a_ret = vImageBuffer_InitWithCGImage(
      &input_buffer, &format, NULL, cgImage, kvImageNoFlags);
  ...
  vImage_Error ret = vImageScale_ARGB8888(&input_buffer, &output_buffer, NULL,
                                          kvImageHighQualityResampling);
  if (ret != kvImageNoError)
    return NULL;

  CGImageRef outputImage = vImageCreateCGImageFromBuffer(
      &output_buffer, &format, NULL, NULL, kvImageNoFlags, &ret);
  if (ret != kvImageNoError) {
...
+ (UIImage *)decodedAndScaledDownImageWithImage:(UIImage *)image
  ...
  // autorelease the bitmap context and all vars to help system to free memory
  // when there are memory warning. on iOS7, do not forget to call
  ...
  @autoreleasepool {
    ...
    // Since the original image here has no alpha info, use
    // kCGImageAlphaNoneSkipFirst to create bitmap graphics contexts without
    // alpha info.
    destContext =
        CGBitmapContextCreate(NULL, destResolution.width, destResolution.height,
                              kBitsPerComponent, 0, colorspaceRef, bitmapInfo);
    ...
    sourceTile.size.height =
        MAX(1, (int)(tileTotalPixels / sourceTile.size.width));
    ...
    // this is the amount of pixels to overlap each tile as we assemble the
    // output image.
    float sourceSeemOverlap = (int)((kDestSeemOverlap / destResolution.height) *
                                    sourceResolution.height);
    CGImageRef sourceTileImageRef;
    // calculate the number of read/write operations required to assemble the
    // output image.
    int iterations = (int)(sourceResolution.height / sourceTile.size.height);
    // If tile height doesn't divide the image height evenly, add another
    // iteration to account for the remaining pixels.
    ...
    for (int y = 0; y < iterations; ++y) {
      @autoreleasepool {
        sourceTile.origin.y =
            y * sourceTileHeightMinusOverlap + sourceSeemOverlap;
        destTile.origin.y =
            destResolution.height -
            ((y + 1) * sourceTileHeightMinusOverlap * imageScale +
             kDestSeemOverlap);
        sourceTileImageRef =
            CGImageCreateWithImageInRect(sourceImageRef, sourceTile);
        if (y == iterations - 1 && remainder) {
          float dify = destTile.size.height;
          destTile.size.height =
              CGImageGetHeight(sourceTileImageRef) * imageScale;
...
#if SD_MAC
    UIImage *destImage =
        [[UIImage alloc] initWithCGImage:destImageRef
                                   scale:image.scale
                             orientation:kCGImagePropertyOrientationUp];
#else
    UIImage *destImage =
...
SDCGContextTransformFromOrientation(CGImagePropertyOrientation orientation,
                                    CGSize size) {
