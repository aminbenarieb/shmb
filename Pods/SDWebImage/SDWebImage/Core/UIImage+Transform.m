...
static inline CGRect SDCGRectFitWithScaleMode(CGRect rect, CGSize size,
                                              SDImageScaleMode scaleMode) {
...
static inline UIColor *SDGetColorFromPixel(Pixel_8888 pixel,
...
static inline CGImageRef _Nullable SDCreateCGImageFromCIImage(
    CIImage *_Nonnull ciImage) {
...
- (void)sd_drawInRect:(CGRect)rect
...
- (nullable UIImage *)sd_resizedImageWithSize:(CGSize)size
  ...
  if (size.width <= 0 || size.height <= 0)
    return nil;
  SDGraphicsImageRendererFormat *format =
      [[SDGraphicsImageRendererFormat alloc] init];
  format.scale = self.scale;
  SDGraphicsImageRenderer *renderer =
      [[SDGraphicsImageRenderer alloc] initWithSize:size format:format];
  UIImage *image = [renderer imageWithActions:^(CGContextRef _Nonnull context) {
    [self sd_drawInRect:CGRectMake(0, 0, size.width, size.height)
                context:context
              scaleMode:scaleMode
          clipsToBounds:NO];
  }];
...
- (nullable UIImage *)sd_croppedImageWithRect:(CGRect)rect {
  ...
  if (self.CIImage) {
    CGRect croppingRect =
        CGRectMake(rect.origin.x, self.size.height - CGRectGetMaxY(rect),
...
#else
    UIImage *image =
        [[UIImage alloc] initWithCIImage:ciImage
                                   scale:self.scale
...
#else
  UIImage *image =
      [[UIImage alloc] initWithCGImage:croppedImageRef
                                 scale:self.scale
                           orientation:kCGImagePropertyOrientationUp];
#endif
...
- (nullable UIImage *)sd_roundedCornerImageWithRadius:(CGFloat)cornerRadius
  ...
  format.scale = self.scale;
  SDGraphicsImageRenderer *renderer =
      [[SDGraphicsImageRenderer alloc] initWithSize:self.size format:format];
  UIImage *image = [renderer imageWithActions:^(CGContextRef _Nonnull context) {
    ...
    if (borderColor && borderWidth < minSize / 2 && borderWidth > 0) {
      CGFloat strokeInset =
          (floor(borderWidth * self.scale) + 0.5) / self.scale;
      CGRect strokeRect = CGRectInset(rect, strokeInset, strokeInset);
      CGFloat strokeRadius =
          cornerRadius > self.scale / 2 ? cornerRadius - self.scale / 2 : 0;
#if SD_UIKIT || SD_WATCH
      UIBezierPath *path = [UIBezierPath
          bezierPathWithRoundedRect:strokeRect
                  byRoundingCorners:corners
                        cornerRadii:CGSizeMake(strokeRadius, strokeRadius)];
#else
...
- (nullable UIImage *)sd_rotatedImageWithAngle:(CGFloat)angle
  ...
  CGRect newRect =
      CGRectApplyAffineTransform(CGRectMake(0, 0, width, height),
                                 fitSize ? CGAffineTransformMakeRotation(angle)
  ...
  format.scale = self.scale;
  SDGraphicsImageRenderer *renderer =
      [[SDGraphicsImageRenderer alloc] initWithSize:newRect.size format:format];
  UIImage *image = [renderer imageWithActions:^(CGContextRef _Nonnull context) {
    CGContextSetShouldAntialias(context, true);
    CGContextSetAllowsAntialiasing(context, true);
    CGContextSetInterpolationQuality(context, kCGInterpolationHigh);
    CGContextTranslateCTM(context, +(newRect.size.width * 0.5),
    ...
    [self
        drawInRect:CGRectMake(-(width * 0.5), -(height * 0.5), width, height)];
  }];
  return image;
}
...
- (nullable UIImage *)sd_flippedImageWithHorizontal:(BOOL)horizontal
  ...
  if (self.CIImage) {
    ...
    if (horizontal) {
      CGAffineTransform flipHorizontal =
          CGAffineTransformMake(-1, 0, 0, 1, width, 0);
...
- (nullable UIImage *)sd_tintedImageWithColor:(nonnull UIColor *)tintColor {
  ...
  if (self.CIImage) {
    CIImage *ciImage = self.CIImage;
    CIImage *colorImage =
        [CIImage imageWithColor:[[CIColor alloc] initWithColor:tintColor]];
...
- (nullable UIColor *)sd_colorAtPoint:(CGPoint)point {
  ...
  size_t components =
      CGImageGetBitsPerPixel(imageRef) / CGImageGetBitsPerComponent(imageRef);
...
- (nullable NSArray<UIColor *> *)sd_colorsWithRect:(CGRect)rect {
  ...
  if (CGRectGetWidth(rect) <= 0 || CGRectGetHeight(rect) <= 0 ||
      CGRectGetMinX(rect) < 0 || CGRectGetMinY(rect) < 0 ||
      CGRectGetMaxX(rect) > width || CGRectGetMaxY(rect) > height) {
    CGImageRelease(imageRef);
    return nil;
  }
  ...
  size_t start =
      bytesPerRow * CGRectGetMinY(rect) + components * CGRectGetMinX(rect);
  size_t end = bytesPerRow * (CGRectGetMaxY(rect) - 1) +
               components * CGRectGetMaxX(rect);
  ...
  // Convert to color
  CGBitmapInfo bitmapInfo = CGImageGetBitmapInfo(imageRef);
  NSMutableArray<UIColor *> *colors = [NSMutableArray
      arrayWithCapacity:CGRectGetWidth(rect) * CGRectGetHeight(rect)];
  for (size_t index = start; index < end; index += 4) {
    ...
    Pixel_8888 pixel = {pixels[index], pixels[index + 1], pixels[index + 2],
                        pixels[index + 3]};
...
// We use vImage to do box convolve for performance and support for watchOS.
// However, you can just use `CIFilter.CIGaussianBlur`. For other blur effect,
// use any filter in `CICategoryBlur`
- (nullable UIImage *)sd_blurredImageWithRadius:(CGFloat)blurRadius {
  ...
  if (self.CIImage) {
    ...
    ciImage = [ciImage imageByCroppingToRect:CGRectMake(0, 0, self.size.width,
                                                        self.size.height)];
#if SD_UIKIT
    UIImage *image = [UIImage imageWithCIImage:ciImage
                                         scale:self.scale
  ...
  vImage_CGImageFormat format = {
      ...
      .bitmapInfo = kCGImageAlphaPremultipliedFirst |
                    kCGBitmapByteOrder32Host, // requests a BGRA buffer.
      .version = 0,
      .decode = NULL,
      .renderingIntent = kCGRenderingIntentDefault};

  vImage_Error err;
  err = vImageBuffer_InitWithCGImage(&effect, &format, NULL, imageRef,
                                     kvImageNoFlags);
  if (err != kvImageNoError) {
    NSLog(@"UIImage+Transform error: vImageBuffer_InitWithCGImage returned "
          @"error code %zi for inputImage: %@",
          err, self);
    return nil;
  }
  err = vImageBuffer_Init(&scratch, effect.height, effect.width,
                          format.bitsPerPixel, kvImageNoFlags);
  if (err != kvImageNoError) {
    NSLog(@"UIImage+Transform error: vImageBuffer_Init returned error code %zi "
          @"for inputImage: %@",
          err, self);
    return nil;
  }
  ...
  if (hasBlur) {
    ...
    if (blurRadius * scale < 0.5)
      ...
      iterations = 3;
    NSInteger tempSize = vImageBoxConvolve_ARGB8888(
        input, output, NULL, 0, 0, radius, radius, NULL,
        kvImageGetTempBufferSize | kvImageEdgeExtend);
    void *temp = malloc(tempSize);
    for (int i = 0; i < iterations; i++) {
      vImageBoxConvolve_ARGB8888(input, output, temp, 0, 0, radius, radius,
  ...
  effectCGImage = vImageCreateCGImageFromBuffer(input, &format, NULL, NULL,
  ...
  free(output->data);
#if SD_UIKIT || SD_WATCH
  UIImage *outputImage = [UIImage imageWithCGImage:effectCGImage
                                             scale:self.scale
                                       orientation:self.imageOrientation];
#else
  UIImage *outputImage =
      [[UIImage alloc] initWithCGImage:effectCGImage
                                 scale:self.scale
                           orientation:kCGImagePropertyOrientationUp];
#endif
