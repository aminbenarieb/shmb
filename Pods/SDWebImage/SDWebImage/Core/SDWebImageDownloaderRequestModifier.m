...
@property(nonatomic, copy, nonnull)
    SDWebImageDownloaderRequestModifierBlock block;
...
+ (instancetype)requestModifierWithBlock:
    (SDWebImageDownloaderRequestModifierBlock)block {
  SDWebImageDownloaderRequestModifier *requestModifier =
      [[SDWebImageDownloaderRequestModifier alloc] initWithBlock:block];
...
- (instancetype)initWithMethod:(NSString *)method
  ...
  body = [body copy];
  return [self
      initWithBlock:^NSURLRequest *_Nullable(NSURLRequest *_Nonnull request) {
