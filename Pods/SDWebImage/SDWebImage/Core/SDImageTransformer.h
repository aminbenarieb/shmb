 ...
 */
FOUNDATION_EXPORT NSString *_Nullable SDTransformedKeyForKey(
    NSString *_Nullable key, NSString *_Nonnull transformerKey);

/**
 Return the thumbnailed cache key which applied with specify thumbnailSize and
 ...
 @note If you have both transformer and thumbnail applied for image, call
 `SDThumbnailedKeyForKey` firstly and then with `SDTransformedKeyForKey`.`
 */
FOUNDATION_EXPORT
...
/**
 ...
 You can provide transformer to cache and manager (Through the `transformer`
 property or context option `SDWebImageContextImageTransformer`).

 @note The transform process is called from a global queue in order to not to
...
@required
/**
 For each transformer, it must contains its cache key to used to store the image
 cache or query from the cache. This key will be appened after the original
 cache key generated by URL or from user.

 @return The cache key to appended after the original cache key. Should not be
 nil.
 */
...
/**
 ...
 @param key The cache key associated to the image. This arg is a hint for image
 source, not always useful and should be nullable. In the future we will remove
 this arg.
...
- (nullable UIImage *)transformedImageWithImage:(nonnull UIImage *)image
    ...
    API_DEPRECATED("The key arg will be removed in the future. Update your "
                   "code and don't rely on that.",
...
/**
 Pipeline transformer. Which you can bind multiple transformers together to let
 the image to be transformed one by one in order and generate the final image.
 @note Because transformers are lightweight, if you want to append or arrange
 transformers, create another pipeline transformer instead. This class is
...
+ (nonnull instancetype)transformerWithTransformers:
    (nonnull NSArray<id<SDImageTransformer>> *)transformers;

@end

// There are some built-in transformers based on the `UIImage+Transformer`
// category to provide the common image geometry, image blending and image
// effect process. Those transform are useful for static image only but you can
// create your own to support animated image as well. Because transformers are
// lightweight, these class are considered as immutable.
...
- (nonnull instancetype)init NS_UNAVAILABLE;
+ (nonnull instancetype)transformerWithSize:(CGSize)size
...
- (nonnull instancetype)init NS_UNAVAILABLE;
+ (nonnull instancetype)transformerWithHorizontal:(BOOL)horizontal
...
- (nonnull instancetype)init NS_UNAVAILABLE;
+ (nonnull instancetype)transformerWithAngle:(CGFloat)angle
