...
// We should override init method for `NSBitmapImageRep` to do initialize about
...
- (instancetype)initWithData:(NSData *)data {
  ...
  if (self) {
    ...
    if (CFStringCompare(type, kUTTypeGIF, 0) == kCFCompareEqualTo) {
      ...
      // Which will use 0 when there are no loop count information metadata in
      // GIF data
      NSUInteger loopCount =
          [SDImageGIFCoder imageLoopCountWithSource:imageSource];
      ...
      NSUInteger loopCount =
          [SDImageAPNGCoder imageLoopCountWithSource:imageSource];
      [self setProperty:NSImageLoopCount withValue:@(loopCount)];
      ...
      [self setProperty:NSImageCurrentFrame withValue:@(0)];
      NSUInteger loopCount =
          [SDImageHEICCoder imageLoopCountWithSource:imageSource];
      [self setProperty:NSImageLoopCount withValue:@(loopCount)];
      ...
      [self setProperty:NSImageCurrentFrame withValue:@(0)];
      NSUInteger loopCount =
          [SDImageAWebPCoder imageLoopCountWithSource:imageSource];
      [self setProperty:NSImageLoopCount withValue:@(loopCount)];
    }
  }
  return self;
...
// `NSBitmapImageRep` will use `kCGImagePropertyGIFDelayTime` whenever you call
// `setProperty:withValue:` with `NSImageCurrentFrame` to change the current
// frame. We override it and use the actual
// `kCGImagePropertyGIFUnclampedDelayTime` if need.
- (void)setProperty:(NSBitmapImageRepPropertyKey)property withValue:(id)value {
  ...
  if ([property isEqualToString:NSImageCurrentFrame]) {
    ...
    if (CFStringCompare(type, kUTTypeGIF, 0) == kCFCompareEqualTo) {
      // GIF
      frameDuration = [SDImageGIFCoder frameDurationAtIndex:index
                                                     source:imageSource];
    } else if (CFStringCompare(type, kUTTypePNG, 0) == kCFCompareEqualTo) {
      // APNG
      frameDuration = [SDImageAPNGCoder frameDurationAtIndex:index
                                                      source:imageSource];
    } else if (CFStringCompare(type, kSDUTTypeHEICS, 0) == kCFCompareEqualTo) {
      // HEIC
      frameDuration = [SDImageHEICCoder frameDurationAtIndex:index
                                                      source:imageSource];
    } else if (CFStringCompare(type, kSDUTTypeWebP, 0) == kCFCompareEqualTo) {
      // WebP
      frameDuration = [SDImageAWebPCoder frameDurationAtIndex:index
                                                       source:imageSource];
    }
    if (!frameDuration) {
      return;
