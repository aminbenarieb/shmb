...
+ (instancetype)playerWithProvider:(id<SDAnimatedImageProvider>)provider {
  SDAnimatedImagePlayer *player =
      [[SDAnimatedImagePlayer alloc] initWithProvider:provider];
  return player;
}
...
- (SDDisplayLink *)displayLink {
  if (!_displayLink) {
    _displayLink =
        [SDDisplayLink displayLinkWithTarget:self
...
- (void)setupCurrentFrame {
  ...
  if (!self.currentFrame &&
      [self.animatedProvider isKindOfClass:[UIImage class]]) {
    UIImage *image = (UIImage *)self.animatedProvider;
// Use the poster image if available
#if SD_MAC
    UIImage *posterFrame =
        [[NSImage alloc] initWithCGImage:image.CGImage
                                   scale:image.scale
...
- (void)resetCurrentFrameStatus {
  // These should not trigger KVO, user don't need to receive an `index == 0,
  // image == nil` callback.
  _currentFrame = nil;
...
- (void)stopPlaying {
  [_fetchQueue cancelAllOperations];
  // Using `_displayLink` here because when UIImageView dealloc, it may trigger
  // `[self stopAnimating]`, we already release the display link in
  // SDAnimatedImageView's dealloc method.
  [_displayLink stop];
  // We need to reset the frame status, but not trigger any handle. This can
  // ensure next time's playing status correct.
...
- (void)displayDidRefresh:(SDDisplayLink *)displayLink {
  // If for some reason a wild call makes it through when we shouldn't be
  // animating, bail. Early return!
  if (!self.isPlaying) {
    return;
  }
  ...
  if (self.playbackMode == SDAnimatedImagePlaybackModeReverse) {
    nextFrameIndex = currentFrameIndex == 0
                         ? (totalFrameCount - 1)
                         : (currentFrameIndex - 1) % totalFrameCount;
    ...
    nextFrameIndex =
        self.shouldReverse ? (currentFrameIndex - 1) : (currentFrameIndex + 1);
  ...
  if (!self.bufferMiss) {
    ...
    NSTimeInterval currentDuration =
        [self.animatedProvider animatedImageDurationAtIndex:currentFrameIndex];
    ...
    NSTimeInterval nextDuration =
        [self.animatedProvider animatedImageDurationAtIndex:nextFrameIndex];
    ...
    if (nextFrameIndex == 0) {
      ...
      // if reached the max loop count, stop animating, 0 means loop
      // indefinitely
  ...
  // When buffer miss, means the decode speed is slower than render speed, we
  // fetch current miss frame Or, most cases, the decode speed is faster than
  // render speed, we fetch next frame
  NSUInteger fetchFrameIndex =
      self.bufferMiss ? currentFrameIndex : nextFrameIndex;
  UIImage *fetchFrame;
  SD_LOCK(_lock);
  ...
  if (!fetchFrame && !bufferFull && self.fetchQueue.operationCount == 0) {
    ...
    NSOperation *operation = [NSBlockOperation blockOperationWithBlock:^{
      ...
      UIImage *frame =
          [animatedProvider animatedImageFrameAtIndex:fetchFrameIndex];
...
- (void)calculateMaxBufferCount {
  NSUInteger bytes = CGImageGetBytesPerRow(self.currentFrame.CGImage) *
                     CGImageGetHeight(self.currentFrame.CGImage);
  if (bytes == 0)
    bytes = 1024;
