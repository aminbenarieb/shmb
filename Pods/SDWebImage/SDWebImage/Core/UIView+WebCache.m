...
- (void)setSd_imageURL:(NSURL *_Nullable)sd_imageURL {
  objc_setAssociatedObject(self, @selector(sd_imageURL), sd_imageURL,
...
- (void)setSd_latestOperationKey:(NSString *_Nullable)sd_latestOperationKey {
  objc_setAssociatedObject(self, @selector(sd_latestOperationKey),
...
- (NSProgress *)sd_imageProgress {
  NSProgress *progress =
      objc_getAssociatedObject(self, @selector(sd_imageProgress));
  if (!progress) {
...
- (void)setSd_imageProgress:(NSProgress *)sd_imageProgress {
  objc_setAssociatedObject(self, @selector(sd_imageProgress), sd_imageProgress,
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)sd_internalSetImageWithURL:(nullable NSURL *)url
  ...
  if (!validOperationKey) {
    // pass through the operation key to downstream, which can used for tracing
    // operation or image view class
  ...
  if (url) {
    ...
    if (!manager) {
      ...
      // remove this manager to avoid retain cycle (manger -> loader ->
...
#if SD_UIKIT || SD_MAC
          if ([imageIndicator
                  respondsToSelector:@selector(updateIndicatorProgress:)]) {
            double progress = 0;
            if (expectedSize != 0) {
    ...
    @weakify(self);
    id<SDWebImageOperation> operation = [manager
        loadImageWithURL:url
               ...
               completed:^(UIImage *image, NSData *data, NSError *error,
                 ...
                 if (imageProgress && finished && !error &&
                     imageProgress.totalUnitCount == 0 &&
                     imageProgress.completedUnitCount == 0) {
                   imageProgress.totalUnitCount =
                       SDWebImageProgressUnitCountUnknown;
                   imageProgress.completedUnitCount =
                       SDWebImageProgressUnitCountUnknown;
                 }
                 ...
                 BOOL shouldCallCompletedBlock =
                     finished || (options & SDWebImageAvoidAutoSetImage);
                 BOOL shouldNotSetImage =
                     ((image && (options & SDWebImageAvoidAutoSetImage)) ||
                 ...
                 // case 1a: we got an image, but the
                 // SDWebImageAvoidAutoSetImage flag is set OR case 1b: we got
                 // no image and the SDWebImageDelayPlaceholder is not set
                 if (shouldNotSetImage) {
                 ...
                 if (image) {
                   // case 2a: we got an image and the
                   // SDWebImageAvoidAutoSetImage is not set
                   targetImage = image;
                   targetData = data;
...
- (void)sd_setImage:(UIImage *)image
  ...
  if (setImageBlock) {
    ...
    finalSetImageBlock = ^(UIImage *setImage, NSData *setImageData,
                           SDImageCacheType setCacheType, NSURL *setImageURL) {
        ...
        animations:^{
          if (!view.sd_latestOperationKey ||
              ![originalOperationKey
                  isEqualToString:view.sd_latestOperationKey]) {
            return;
          }
...
#pragma clang diagnostic pop
                if (!timingFunction) {
                  timingFunction = SDTimingFunctionFromAnimationOptions(
                ...
                CATransition *trans = SDTransitionFromAnimationOptions(
                    transition.animationOptions);
                if (trans) {
...
#elif SD_WATCH
  // Do nothing because WatchKit automatically layout the view after property
  // change
#endif
}
...
- (void)setSd_imageTransition:(SDWebImageTransition *)sd_imageTransition {
  objc_setAssociatedObject(self, @selector(sd_imageTransition),
                           sd_imageTransition,
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)setSd_imageIndicator:(id<SDWebImageIndicator>)sd_imageIndicator {
  ...
  objc_setAssociatedObject(self, @selector(sd_imageIndicator),
                           sd_imageIndicator,
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
#if SD_MAC
  [view
      setFrameOrigin:CGPointMake(
                         round((NSWidth(self.bounds) - NSWidth(view.frame)) /
                               2),
                         round((NSHeight(self.bounds) - NSHeight(view.frame)) /
                               2))];
#else
  view.center =
