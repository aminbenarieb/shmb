...
//            .store(in: &self.cancelableSet)
    ...
    private func fetch() {
        ...
        self.persistentStore.fetch(fetchStocksInfo: .init(page: 1, isFavourite: nil))
            .sink { [weak self] c in
                ...
                switch c {
                case let .failure(error):
                    self.state = .main(.error(.init(localizedDescription: error.localizedDescription)))
                case .finished:
                    self.state = .main(self.watchListPage.data.isEmpty ? .empty(EmptyInfo()) : .data(self.watchListPage.data))
                }
    ...
    //            }
                .sink { [weak self] completion in
                    ...
                    guard let self = self else { return }
                    self.searchListPage = .init(data: result.value, page: 1)
                }
